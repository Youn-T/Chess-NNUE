...
labels = cp.load('labels_halfKP.npz')["Y"]
matrix_cpu = sparse.load_npz('moves_halfKP.npz')
...
for epoch in range(EPOCHS):
    ...
    for i in range(0, len(indices), BATCH_SIZE):
        ...
        # ==========================================================
        # 1. Mise à jour des moments (Dense)
        mW[0] = BETA1 * mW[0] + (1 - BETA1) * dWs[0]
        vW[0] = BETA2 * vW[0] + (1 - BETA2) * (dWs[0]**2)
        mb[0] = BETA1 * mb[0] + (1 - BETA1) * dbs[0] # <--- NE PAS OUBLIER
        vb[0] = BETA2 * vb[0] + (1 - BETA2) * (dbs[0]**2) # <--- NE PAS OUBLIER

        # 2. Correction du biais (Bias Correction)
        mw_hat_0 = mW[0] / (1 - BETA1**t)
        vw_hat_0 = vW[0] / (1 - BETA2**t)
        mb_hat_0 = mb[0] / (1 - BETA1**t)
        vb_hat_0 = vb[0] / (1 - BETA2**t)

        # 3. Mise à jour des poids et biais
        weights[0] -= current_lr * (mw_hat_0 / (cp.sqrt(vw_hat_0) + EPSILON) + LAMBDA_L2 * weights[0])
...
cp.savez('model_halfKP.npz', W1=W1, b1=b1, W2=W2, b2=b2, W3=W3, b3=b3, W4=W4, b4=b4)
print("Modèle sauvegardé sous 'model_halfKP.npz'")